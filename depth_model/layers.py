...
class DetailGuide(nn.Module):
    ...
    def forward(self, height_re, width_re, dxy):
        ...
        for i in range(self.batch_size):
            ...
            #2025-5-10：删除 ones = nn.Parameter(torch.ones(1, 1, self.height * self.width),
            #2025-5-10：删除                         requires_grad=False).cuda()   #[1,1,122880]
            ones = nn.Parameter(torch.ones(1, 1, self.height * self.width, device=dxy.device),#2025-5-10：增加
                                     requires_grad=False)#2025-5-10：增加

            pix_coords = nn.Parameter(torch.unsqueeze(torch.stack(
                #2025-5-10：删除    [id_coords[0].view(-1), id_coords[1].view(-1)], 0), 0), requires_grad=False).cuda()    #[1,2,122880]
            #2025-5-10：删除# self.pix_coords = self.pix_coords.repeat(self.batch_size, 1, 1)  #[12,2,122880]
                [id_coords[0].view(-1), id_coords[1].view(-1)], 0), 0), requires_grad=False).to(dxy.device)#2025-5-10：增加
...
class BackprojectDepth(nn.Module):
    ...
    def forward(self, depth, inv_K, dxy):
        ...
        for i in range(self.batch_size):
            ...
            #2025-5-10：删除pix_coords = torch.unsqueeze(torch.stack(
            #2025-5-10：删除    [id_coords[0].view(-1), id_coords[1].view(-1)], 0), 0).cuda()    #[1,2,122880]
            #2025-5-10：删除# self.pix_coords = self.pix_coords.repeat(self.batch_size, 1, 1)  #[12,2,122880]
            pix_coords = nn.Parameter(torch.cat([pix_coords, ones], 1),
                                      #2025-5-10：删除                               requires_grad=False).cuda()   #[1,3,122880]
                                      requires_grad=False).to(dxy.device)#2025-5-10：增加
