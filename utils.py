...
def get_files(path):
    ...
    for root, dirs, files in os.walk(path_rainy):
        ...
        for name in files:
            if name.split('.')[1] != 'png' and name.split('.')[1] != 'jpg':
                continue
...
# for depth
def get_files_depth(path):
    ret = []
    path_rainy = path + "image"
    path_gt = path + "mask"
    path_depth = path + "depth_img"
    ...
    for root, dirs, files in os.walk(path_rainy):
        ...
        for name in files:
            ...
            file_rainy = path_rainy + "/" + name
            file_gt = path_gt + "/" + name
            file_depth = path_depth + "/" + name
            ret.append([file_rainy, file_gt,file_depth])
            # print("......................",ret)
    return ret
