...
def create_dataloader(dataset, dataset_opt, opt=None, sampler=None):
    ...
    if phase == "train":
        if opt["dist"]:
            world_size = torch.distributed.get_world_size()   # 它返回当前进程所在的运行环境中的进程总数
        ...
        print("tttttttttttttrrrrrrrrrrrrraaaaaaaaiiiiiiiiinnnnnnnngggggggaaaaannnnnnnnnn")
        if opt["dist"]:
            world_size = torch.distributed.get_world_size()   # 它返回当前进程所在的运行环境中的进程总数
            num_workers = dataset_opt["n_workers"]
            assert dataset_opt["batch_size"] % world_size == 0
...
def create_dataset(dataset_opt):
    ...
    elif mode == "LQGT":  # SFTMD============================================= used (存在lq和gt图像)
