...
def imresize(img, scale, antialiasing=True):
    ...
    if isinstance(img, np.ndarray):
        #2025-5-10：删除 img = torch.from_numpy(img.transpose(2, 0, 1)).cuda()
        img = torch.from_numpy(img.transpose(2, 0, 1))#2025-5-10：增加
        is_numpy = True
    #2025-5-10：删除 device = img.device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")#2025-5-10：增加
    img = img.to(device)#2025-5-10：增加
