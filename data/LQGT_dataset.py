...
class LQGTDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.LR_paths, self.GT_paths, self.LRdepth_paths, self.GTdepth_paths = None, None, None, None
        self.LR_env, self.GT_env, self.LRdepth_env, self.GTdepth_env = None, None, None, None  # environment for lmdb
        ...
        if opt["data_type"] == "lmdb":
            ...
            self.LRdepth_paths, self.LR_sizes = util.get_image_paths(
                opt["data_type"], opt["dataroot_LQdepth"]
            )
            self.GTdepth_paths, self.GT_sizes = util.get_image_paths(
                opt["data_type"], opt["dataroot_GTdepth"]
            )
            ...
            self.LRdepth_paths = util.get_image_paths(
                opt["data_type"], opt["dataroot_LQdepth"]
            )  # LR list
            self.GTdepth_paths = util.get_image_paths(
                opt["data_type"], opt["dataroot_GTdepth"]
            )  # GT list
        else:
    ...
    def _init_lmdb(self):
        ...
        self.GTdepth_env = lmdb.open(
            self.opt["dataroot_GTdepth"],
            readonly=True,
            lock=False,
            readahead=False,
            meminit=False,
        )
        self.LRdepth_env = lmdb.open(
            self.opt["dataroot_LQdepth"],
            readonly=True,
            lock=False,
            readahead=False,
            meminit=False,
    ...
    def __getitem__(self, index):
        ...
        GT_path, LR_path, GTdepth_path, LRdepth_path = None, None, None, None
        ...
        # get GT image
        GTdepth_path = self.GT_paths[index]
        if self.opt["data_type"] == "lmdb":
            resolution = [int(s) for s in self.GT_sizes[index].split("_")]
        else:
            resolution = None
        img_GTdepth = util.read_img(
            self.GTdepth_env, GTdepth_path, resolution
        )  # return: Numpy float32, HWC, BGR, [0,1]

        # get GT image
        LRdepth_path = self.GT_paths[index]
        if self.opt["data_type"] == "lmdb":
            resolution = [int(s) for s in self.GT_sizes[index].split("_")]
        else:
            resolution = None
        img_LRdepth = util.read_img(
            self.LRdepth_env, LRdepth_path, resolution
        )  # return: Numpy float32, HWC, BGR, [0,1]
        ...
        if self.opt["phase"] == "train":
            ...
            img_LRdepth = img_LRdepth[rnd_h: rnd_h + LR_size, rnd_w: rnd_w + LR_size, :]
            rnd_h_GT, rnd_w_GT = int(rnd_h * scale), int(rnd_w * scale)
            img_GTdepth = img_GTdepth[
                     rnd_h_GT: rnd_h_GT + GT_size, rnd_w_GT: rnd_w_GT + GT_size, :
                     ]
            ...
            img_LRdepth, img_GTdepth = util.augment(
                [img_LRdepth, img_GTdepth],
                self.opt["use_flip"],
                self.opt["use_rot"],
                self.opt["mode"],
                self.opt["use_swap"],
        ...
        if img_GT.shape[2] == 3:
            img_GT = img_GT[:, :, [2, 1, 0]]
            img_LR = img_LR[:, :, [2, 1, 0]]
            # [BUG修复 2025-12-27] 原代码错误地使用 img_GT 和 img_LR 覆盖了深度图
            # 现在正确使用 img_GTdepth 和 img_LRdepth
            img_GTdepth = img_GTdepth[:, :, [2, 1, 0]]
            img_LRdepth = img_LRdepth[:, :, [2, 1, 0]]
        ...
        # print("=============================gtdepth=========================================")
        img_GTdepth = torch.from_numpy(
            np.ascontiguousarray(np.transpose(img_GTdepth, (2, 0, 1)))
        ).float()
        # print("=============================gtdepth=========================================")
        img_LRdepth = torch.from_numpy(
            np.ascontiguousarray(np.transpose(img_LRdepth, (2, 0, 1)))
        ).float()
        ...
        return {"LQ": img_LR, "GT": img_GT, "LQ_path": LR_path, "GT_path": GT_path,
                "LQdepth": img_LRdepth, "GTdepth": img_GTdepth, "LQdepth_path": LRdepth_path,
                "GTdepth_path": GTdepth_path}
